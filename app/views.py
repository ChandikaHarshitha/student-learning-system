...
def displayformdatafunction(request):
    if request.method == "POST":
        ...
        return render(request, "displayform.html",
                      {"id": id, "name": name, "gender": gender, "dob": dateofbirth, "dept": department,
                       "email": emailid, "contactno": contactno})
...
def checkuserlogin(request):
    ...
    flag = Student.objects.filter(Q(email=emailid) & Q(
        password=pwd))  # faculty_registration.objects.filter(Q(f_email=emailid) & Q(f_password=pwd)) )
    print(flag)
...
def leaderboard(request):
    ...
    top_students = Marks.objects.values('student__username').annotate(total_marks=Sum('marks_obtained')).order_by(
        '-total_marks')[:10]
